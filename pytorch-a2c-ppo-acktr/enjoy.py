...
while True:
    with torch.no_grad():
        if args.sf:
            q, psi, action, _, recurrent_hidden_states = actor_critic.act(
            obs, recurrent_hidden_states, masks, feature, deterministic=False)
        else:
    ...
    feature = torch.FloatTensor(np.stack(_feature, axis = 0))
